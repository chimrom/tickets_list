...
export const StopsFilter = ({ stopsCount, setSelected, selected }) => {
				...
				<label className="relative ">
					...
					<span
						className={`absolute top-0 left-0 w-4 h-4 bg-white border  rounded ${
							Boolean(selected.length === stopsCount.length)
								? 'border-blue-500'
								: 'border-gray-300'
						}`}
					>
						...
						>
							...
							>
								...
								>
									...
									<span
										className={`absolute top-0 left-0 w-4 h-4 bg-white border  rounded ${
											Boolean(selected?.find((el) => el === stop))
												? 'border-blue-500'
												: 'border-gray-300'
										}`}
									>
									...
									<p className="ml-6 text-gray-600">{getStopByCount(stop)}</p>
								</label>
